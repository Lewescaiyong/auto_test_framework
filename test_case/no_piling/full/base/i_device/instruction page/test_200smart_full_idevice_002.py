...
class Test200SmartFullIdevice002(CaseMW):
    ...
    def process(self):
        ...
        self.logger.info('1. In system block, select the CPU module is "ST60", version is V02.04.00, '
                         'click "PROFINET" in wizard;')
        system_block = self.PROJECT.find('system_block')
        system_block.set_module_info({'cpu': {'cpu_type': 'ST60', 'cpu_version': '02.04'}})
        ...
        if not (pn_support and (not i_device_support) and controller_support):
            self.logger.info('pn_support: %s, i_device_support: %s, controller_support: %s.' % (
                pn_support, i_device_support, controller_support))
            raise CheckException('1. The "PROFINET" is grey or in PN wizard, not only "Controller" under "PLC Role";')
