...
class Test200SmartFullIdevice074(CaseMW):
    """Controller +  i-device (fixed IP)(catalog configuration)
    No.: test_200smart_full_idevice_074
    Preconditions:
        1. Open Micro/WIN V2.5;
        2. Connect Micro/WIN to PLC;
        3. Select plc role is "idevice" in PN wizard;
    Step actions:
        1. Config fixed ip and name, add transfer area,  then generate and download;
        2. View configuration in plc information, then power cycle the PLC, and view the config;
        3. Another PLC, select role is controller,add idevice in step1 by catalog configuration, generate and download;
    ...
    def prepare(self):
        ...
        ChangeInfo: Cai, Yong 2019-12-13 create
        """
        super(Test200SmartFullIdevice074, self).prepare()
    ...
    def process(self):
        ...
        ChangeInfo: Cai, Yong 2019-12-13 create
        """
        super(Test200SmartFullIdevice074, self).process()

        self.logger.info('Step actions:')
        self.logger.info('1. Config fixed ip and name, add transfer area,  then generate and download;')
        self.logger.info('2. View configuration in plc information, then power cycle the PLC, and view the config;')
        self.logger.info('3. Another PLC, select role is controller,add idevice in step1 by catalog configuration, generate and download;')
    ...
    def cleanup(self):
        ...
        ChangeInfo: Cai, Yong 2019-12-13 create
        """
        super(Test200SmartFullIdevice074, self).cleanup()
