...
class Test200SmartFullIdevice080(CaseMW):
    """Controller + 8 i-device(with low level system)(with 8 io-device)
    No.: test_200smart_full_idevice_080
    Preconditions:
        1. Open Micro/WIN V2.5;
        2. Connect Micro/WIN to PLC;
        3. Select plc role is "controller & idevice" in PN wizard;
    Step actions:
        1. Config ip and name, add transfer area, export gsdml file,  then add 8 io-devices, generate and download for 8 plc;
        2. View configuration in plc information, then power cycle the PLC, and view the config;
        3. Another PLC, import gsdml file in step1,  select role is controller,add idevices in step1 and 7 io-device, generate and download;
    ...
    def prepare(self):
        ...
        ChangeInfo: Cai, Yong 2019-12-13 create
        """
        super(Test200SmartFullIdevice080, self).prepare()
    ...
    def process(self):
        ...
        ChangeInfo: Cai, Yong 2019-12-13 create
        """
        super(Test200SmartFullIdevice080, self).process()

        self.logger.info('Step actions:')
        self.logger.info('1. Config ip and name, add transfer area, export gsdml file,  then add 8 io-devices, generate and download for 8 plc;')
        self.logger.info('2. View configuration in plc information, then power cycle the PLC, and view the config;')
        self.logger.info('3. Another PLC, import gsdml file in step1,  select role is controller,add idevices in step1 and 7 io-device, generate and download;')
    ...
    def cleanup(self):
        ...
        ChangeInfo: Cai, Yong 2019-12-13 create
        """
        super(Test200SmartFullIdevice080, self).cleanup()
